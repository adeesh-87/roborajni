...
1. For each G row: the card, `index.sh source "$KB_DIR/index" <function>` and `index.sh flow "$KB_DIR/index" <function>`
   (the path to the NOT HIT edge). Walk the conditions back to the function entry; write the exact inputs, state and
   mock returns needed as a `Cases` line.
