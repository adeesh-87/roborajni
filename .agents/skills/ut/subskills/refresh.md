...
It rebuilds the graph with the arguments of the last build, rescans the tests and regenerates every module's cards.
Hand-written notes (`modules/<name>.md`, `exemplars/`, `kb.md`) are never touched by the script.

1. (parallel) `"$LOCK" "$LD" wait $ME 900 "$KB_DIR/graphify" "$KB_DIR/modules" "$KB_DIR/testscan.md" "$KB_DIR/last-refresh.md"`
2. `"$SKILL_DIR/resources/scripts/graphify.sh" refresh "$KB_DIR/graphify"`   (about a minute; prints the delta)
