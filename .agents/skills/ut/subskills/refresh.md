...
It rebuilds the code index with the arguments of the last build, rescans the tests and regenerates every module's cards and the diagrams.
...
2. `"$SKILL_DIR/resources/scripts/index.sh" refresh "$KB_DIR/index"`   (about a minute; rebuilds the Graphify index, regenerates cards and diagrams, prints the delta)
